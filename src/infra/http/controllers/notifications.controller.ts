...
export class NotificationsController {
  ...
  async getFromRecipient(@Param('recipientId') recipientId: string) {
    const { notificationsList } = await this.getRecipientNotification.execute({ recipientId });

    return { notificationsList: notificationsList.map(NotificationViewModel.toHTTP) };
  }
